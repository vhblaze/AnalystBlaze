...
function Show-DevServiceWarning {
  ...
  if ($service -and $service.PathName -and $service.PathName.IndexOf($targetPrefix, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
    Write-Warning "AnalystBlazeHelper is installed from the dev target and can lock checks. Remove it from an elevated PowerShell: sc.exe stop AnalystBlazeHelper; sc.exe delete AnalystBlazeHelper"
  }
}
...
try {
  for ($attempt = 1; $attempt -le $MaxAttempts; $attempt++) {
    Write-Host "cargo check attempt $attempt/$MaxAttempts using target $TargetDir"
    Show-DevServiceWarning -Path $TargetDir
    cargo check
    if ($LASTEXITCODE -eq 0) {
      Write-Host "cargo check finished successfully."
      exit 0
    }
  ...
  Write-Error "cargo check did not finish after $MaxAttempts attempts."
  exit 1
} finally {
  Pop-Location
}
